...
export class ProductsService {
  ...
  async create(data: ProductDTO[]) {
    try {
      for (const productData of data) {
        ...
        const productmodelEntity = new ProductModel({
          modelName: productData.productModel.modelName,
        });
        const productModel = await this.productModelRepository.findOneOrCreate(
          { modelName: productData.productModel.modelName },
          productmodelEntity,
        );
        const sizes = [];
        ...
        for (const sizeV of productData.sizes) {
          ...
          const size = await this.sizeRepository.findOneOrCreate(
            { sizeValue },
            sizeEntity,
          );
          sizes.push(size);
  ...
  async findProductsByQueryParams(query: QueryValidator) {
    const { model, size, brand, subCategory, category } = query;
    if (model || size || brand || subCategory || category) {
      return this.productRepository.findProductsByQueryParams(
        model,
        size,
        brand,
        subCategory,
