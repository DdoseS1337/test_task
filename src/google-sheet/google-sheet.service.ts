...
export class GoogleSheetService {
  ...
  async getSheetData(): Promise<any> {
    console.log("server starting update data...");
    try {
    // Отримуємо дані про сторінки в гугл документі
      ...
      if (sheetsList) {
        const sheetTitles = sheetsList.map(
          (sheet: any) => sheet.properties?.title,
        );
        // Отримуємо дані про кожен лист
        const dataPromises = sheetTitles.map((sheetTitle: string) =>
          this.getDataFromGoogleSheet(sheetTitle),
        );
    ...
    try {
      ...
      productsNames.forEach((productName, index) => {
        ...
        products.push({
          ...
          productModel: { modelName: sheetName, brandName: null },
        });
      });
